...
import { ChatMessage, ChatMessageGroup, MessageSource } from './message.types';
...
export class ChatService {
  ...
  registerChatService(chatService: ChatServiceBase) {
  ...
  async message(content: string) {
    ...
    try{
      ...
      await Promise.all([...this.chatServices.values()].map((s) => s.sendMessage()));
    }
    finally{
