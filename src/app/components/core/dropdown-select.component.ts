...
@Component({
  ...
  template: ` <div
    ...
    class="w-10rem p-1 z-3 border-round border-1 border-primary-200 flex flex-row justify-content-between"
  >
    <div
    ...
    >
      ...
      <p-overlayPanel
        #op
        styleClass="overlay-child-p-0 shadow-none"
          ...
          <ng-template let-item pTemplate="item">
            ...
            >
              <p-checkbox
                ...
                (onChange)="
                  selectedChanges.emit({ all: items, current: [item] })
                "
                [binary]="true"
                [inputId]="item[labelName]"
              ></p-checkbox>
              <label [for]="item[labelName]">{{ item[labelName] }}</label>
              <div class="flex flex-grow-1 justify-content-end">
                <i
                  *ngIf="item['link']"
                  class="pi pi-external-link text-300 hover:text-700"
    ...
    <div
      class="w-2rem flex justify-content-center align-items-center text-primary"
    >
      <i
        [class.rotate-180]="isOpened"
        class="pi pi-chevron-down transition-all transition-duration-400"
      ></i>
    </div>
  </div>`,
  styles: `
